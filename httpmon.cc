...
void report(ClientData &_data, AccumulatedData &accData)
{
	...
	printf("time=%.6f latency=%.0f:%.0f:%.0f:%.0f:%.0f:(%.0f)us latency95=%.0fus latency99=%.0fus requests=%d option1=%d option2=%d errors=%d throughput=%.0frps ql=%d rr=%.2f%% cr=%.2f%% accRequests=%d accOption1=%d accOption2=%d accLatency=%.0f:%.0f:%.0f:%.0f:%.0f:(%.0f)us accLatency95=%.0fus accLatency99=%.0fus accOpenQueuing=%d accErrors=%d\n",
		reportTime,
		stats.minimum,
		stats.lowerQuartile,
		stats.median,
		...
		accData.numOption2,
		accStats.minimum,
		accStats.lowerQuartile,
		accStats.median,
		accStats.upperQuartile,
		accStats.maximum,
		accStats.average,
		accStats.percentile95,
		accStats.percentile99,
